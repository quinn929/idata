...
public class JedisUtil {
	 ...
	 * @return JedisPool
	 */
	public static void init(String ip,String part,String password,int timeout,int poolsize) {
		if (jedisPool == null) {
			JedisPoolConfig config = new JedisPoolConfig();
			config.setMaxActive(poolsize);
			config.setMaxIdle(poolsize);
			config.setMaxWait(poolsize/2);
			config.setTestOnBorrow(true);
			config.setTestOnReturn(true);
			if (StringUtils.isBlank(password)){
				jedisPool = new JedisPool(config, ip, Integer.valueOf(part), timeout);
			}else{
				jedisPool = new JedisPool(config, ip, Integer.valueOf(part), timeout, password);
			}
			
//			jedisPool = new JedisPool(config, ip,Integer.valueOf(part));
		}
	}
	...
	public class SortSet {
		 ...
		 * @return Set<String>
		 * */
		public Set<String> zrangeLimit(String key, double min, double max , int offset, int count) {
			Jedis sjedis = getJedis();
